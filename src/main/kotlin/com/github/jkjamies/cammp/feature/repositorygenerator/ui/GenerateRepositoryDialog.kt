...
class GenerateRepositoryDialog(private val project: Project) : DialogWrapper(project) {
    ...
    init {
        ...
        // DI radio grouping and listeners
        ButtonGroup().apply {
            add(rbHilt)
            add(rbKoin)
        }
    ...
    override fun createCenterPanel(): JComponent {
        ...
        panel.preferredSize = JBUI.size(780, 260)
    ...
    private fun updateDiControls() {
        ...
        // Koin Annotations visible only for Koin and DI enabled
    ...
    override fun doValidateAll(): List<ValidationInfo> {
        ...
        if (dir.isBlank()) {
            ...
            val last = dir.trimEnd('/', '\\').substringAfterLast('/')
                .substringAfterLast('\\')
            if (!last.equals("data", ignoreCase = true)) {
                errors += ValidationInfo("Selected directory must be a data module", dirField)
            }
        }
        if (nameField.text.isBlank()) {
            errors += ValidationInfo("Please enter a repository name", nameField)
        }
        return errors
    }

    fun getTargetDir(): String = dirField.text
    fun getRepositoryName(): String = nameField.text.trim()
